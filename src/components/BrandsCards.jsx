...
export default function BrandsCards() { 
  ...
  return ( 
    <section className="relative min-h-[140vh] lg:min-h-screen flex flex-col lg:flex-row items-center justify-center overflow-hidden px-4 md:px-8 py-12 gap-12 lg:gap-0"> 
        
 
       {/* LEFT SLIDER - Full height */} 
       <div className="w-full lg:w-auto lg:absolute lg:left-8 z-10 flex flex-col items-center justify-center h-[70vh] lg:h-full min-h-[500px]"> 
         <Motion.div 
           ref={containerRef} 
        ...
        >
          {loopLeft.map((logo, i) => (
             <Motion.img 
               ...
               src={logo} 
               alt={`brand-${logo}`} 
               className="h-24 w-24 md:h-28 md:w-28 lg:h-36 lg:w-36 xl:h-44 xl:w-44 2xl:h-52 2xl:w-52 object-contain cursor-pointer grayscale hover:grayscale-0 transition-all duration-500 hover:shadow-2xl hover:shadow-white/40 hover:brightness-110 rounded-2xl bg-white/15 p-4 md:p-5 xl:p-6 backdrop-blur-md border-2 border-white/30 hover:border-white/60 flex-shrink-0 opacity-95 hover:opacity-100" 
       ...
       {/* CENTER TITLE */} 
       <Motion.h2 
         className="text-center text-white text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-black z-20 drop-shadow-2xl tracking-tighter w-full lg:w-auto order-3 lg:order-none flex-shrink-0 px-8 lg:px-0" 
      ...
      <div className="w-full lg:w-auto lg:absolute lg:right-8 z-10 flex flex-col items-center justify-center h-[70vh] lg:h-full min-h-[500px]">
        <Motion.div
          className="flex flex-col items-center space-y-4 md:space-y-6 lg:space-y-10 xl:space-y-12 w-full"
          style={{ y: rightY }}
